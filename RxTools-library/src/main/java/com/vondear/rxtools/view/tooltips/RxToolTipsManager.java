...
public class RxToolTipsManager {
    ...
    public View show(RxToolTip rxToolTip, int timeOut) {
        final View tipView = create(rxToolTip);
        if (tipView == null) {
            return null;
        }
        ...
        Runnable timeoutRunnable = new Runnable() {
            @Override
            public void run() {
                dismiss(tipView, false);
            }
        };

        tipView.removeCallbacks(timeoutRunnable);
        tipView.postDelayed(timeoutRunnable, timeOut);
